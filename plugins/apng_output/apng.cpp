...
/// <returns></returns>
u32 calcCrc(u8* pSrc, int byteNum) {
...
u32 readu32be(u8* pSrc, int offset) {
...
int search(u8* pSrc, int byteNum, CHUNK* pChunks, int maxNum) {
	int offset = 0;
	int chunkNum = 0;
	CHUNK* p = pChunks;
...
int makeChunk(u8* pDst, int inOffset, int maxByte) {
	int offset = inOffset;
	// bodybyte
	// tag
	// body
...
HGLOBAL makeMemoryPng(const float* pSrc,
	...
	do {
		...
		do {
			...
			auto chunkNum = search(nullptr, 0, chunks, 256);
			// 
