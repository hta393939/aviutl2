...
#include "jpeghdr_input.h"
#include "../lib/util.hpp"

#define STRBUF (4096)

#define APPNAME "jpeghdr_input"
...
/// </summary>
/// <returns></returns>
int parseJpegHdr(unsigned char* src, int byteNum) {
	unsigned short* p16;
	int count = 0;
	int status = 0;
	for (int i = 0; i < byteNum - 1; ++i) {
		auto p16 = (unsigned short*)(src + i);
		if (*p16 == 0xd8ff) {
			status = 2;
			i += 1;
		}
...
INPUT_PLUGIN_TABLE input_plugin_table = {
	...
	TEXT("連番JPEG-HDR入力"),			//	プラグインの名前
	TEXT("JPEG-HDR File (*.jpg)\0*.jpg\0JPEG-HDR File (*.jpeg)\0*.jpeg\0AllFile (*.*)\0*.*\0"),
	TEXT("連番JPEG-HDR入力 v0.3.1 by ウサギ"),	//	プラグインの情報
